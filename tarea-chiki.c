...
void chooseCommand(){
	int num;
	listCommand();
	puts("Seleccione número del comando que desea volver a ejecutar: \n");
	scanf("%d", &num);
	char archivo[] = "Log/templog.log";
	char *line = NULL;
	FILE *fp;
...
int main (int argc, char *argv[]) {
	...
	int p[2],hay_pipe,i,temp,mishell_log,templog;
	...
	while(1) {		 	
		...
		templog= open("Log/templog.log",O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IRGRP | S_IWGRP | S_IWUSR);	
		write(templog,input,strlen(input)); //se guarda el comando leído en el log
		close(templog);
